...
trellato.controller 'mainCtrl', 
($scope, trello, global, getLists, storage, $rootScope) ->
  $scope.trello = trello

  loadBoards = ->
    trello.get "organizations/#{ window.orgId }/boards/?filter=open", 
    (boards) ->
...
trellato.directive 'tasklist', ->
  ...
  template: '''
    <div class='tasklist' ng-mouseenter='expand()' ng-mouseleave='collapse()'
      ...
      <listname></listname>
      <div class='listcards' 
        ng-class='{stacked: !expanded && options.enableStacking}'>
